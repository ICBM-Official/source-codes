...
async def bannedusers(ctx, *, page: int = 1):
    ...
    for ban in banned_users:
        ...
        name = ban[1].name
        id = ban[1].id
        value = (f'{name} {id}. Reason = {reason}')
        member_list.append(value)


    if len(member_list) == 0:
        embed = (discord.Embed(description='No banned users'))
    ...
    embed = (discord.Embed(description='**{} Banned Users:**\n\n{}'.format(len(member_list), bmembers))
    ...
    while True:
        try:
            ...
            if str(reaction.emoji) == "▶️" and page != pages:
                ...
                for n, i in enumerate(member_list[start:end], start=start):
                    bmembers += (f'{n+1}, <@{i}>\n')
                newembed = (discord.Embed(description='**{} Banned Users:**\n\n{}'.format(len(member_list), bmembers)))
